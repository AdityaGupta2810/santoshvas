...
// Database operations
// $dbn = mysqli_connect($host, $username, $password, "santoshvastralay"); // Connect to database
...
if (!empty($searchTerm)) {
    ...
    $searchCondition = " WHERE LOWER(size_name) LIKE '%$searchTerm%'";
}

// Count total records for pagination
$countQuery = "SELECT COUNT(*) as total FROM tbl_size" . $searchCondition;
$countResult = mysqli_query($db, $countQuery);
$totalRecords = mysqli_fetch_assoc($countResult)['total'];
$totalPages = ceil($totalRecords / $itemsPerPage);

// Get records for current page with sorting
$query = "SELECT * FROM tbl_size" . $searchCondition . " ORDER BY $sortColumn $sortOrder LIMIT $offset, $itemsPerPage";
$result = mysqli_query($db, $query);

// Handle size deletion if requested
if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
    $delete_id = (int)$_GET['delete_id'];
    $delete_query = "DELETE FROM tbl_size WHERE id = $delete_id";
...
<div class="container mx-auto p-4">
    ...
    <!-- Controls and search -->
    <form method="GET" action="size.php" class="flex flex-col sm:flex-row justify-between items-center mb-4">
      ...
      <div class="flex items-center w-full sm:w-auto">
        ...
        <input type="text" name="search" id="searchInput" class="border rounded px-2 py-1 w-full sm:w-64" 
               value="<?php echo htmlspecialchars($searchTerm); ?>" placeholder="Search by size name..." />
        ...
        <thead>
          <tr class="bg-gray-50">
            ...
            <th class="py-2 px-4 border text-left relative">
              <a href="<?php echo getSortUrl('size_name', $sortColumn, $sortOrder); ?>" class="flex items-center justify-between">
                size Name
                <span class="ml-1">
                  <?php echo getSortIcon('size_name', $sortColumn, $sortOrder); ?>
                </span>
              </a>
            </th>
            <th class="py-2 px-4 border text-left relative">
        ...
        <tbody id="sizeTableBody">
          ...
          if (mysqli_num_rows($result) > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                  ...
                  echo '<td class="py-2 px-4 border">' . htmlspecialchars($row['size_name']) . '</td>';
                  echo '<td class="py-2 px-4 border flex flex-col sm:flex-row">';
                  echo '<a href="size-edit.php?id=' . $row['id'] . '" class="bg-blue-500 text-white px-3 py-1 rounded mb-1 sm:mb-0 sm:mr-1 text-center hover:bg-blue-600">';
                  echo '<i class="fas fa-edit mr-1"></i> Edit</a>';
                  echo '<a href="size.php?delete_id=' . $row['id'] . '" class="bg-red-500 text-white px-3 py-1 rounded text-center hover:bg-red-600" ';
                  echo 'onclick="return confirm(\'Are you sure you want to delete this size?\')"><i class="fas fa-trash mr-1"></i> Delete</a>';
                  echo '</td>';
                  echo '</tr>';
              }
          } else {
              echo '<tr><td colspan="3" class="py-4 px-4 text-center">No sizes found</td></tr>';
          }
          ?>
        </tbody>
      </table>
