...
$allowedColumns = ['ecat_id','ecat_name', 'mcat_id', 'tcat_id'];
...
if (!empty($searchTerm)) {
    ...
    $searchCondition = " WHERE LOWER(ecat_name) LIKE '%$searchTerm%'";
}

// Count total records for pagination

$countQuery = "SELECT COUNT(*) as total FROM tbl_end_category" . $searchCondition;
$countResult = mysqli_query($db, $countQuery);
$totalRecords = mysqli_fetch_assoc($countResult)['total'];
$totalPages = ceil($totalRecords / $itemsPerPage);

// Get records for current page with sorting  ////////////
$query = "Select e.*, m.mcat_name, t.tcat_name FROM tbl_end_category e 
...
if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM tbl_end_category WHERE ecat_id = $delete_id";
...
<div class="container mx-auto p-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
      <h1 class="text-2xl font-semibold flex items-center mb-3 sm:mb-0">
        <i class="fas fa-circle-dot mr-2"></i> View End Level Categoriess
      </h1>
      <button id="addNewBtn" class="bg-gray-800 text-white px-4 py-2 rounded hover:bg-gray-700 w-full sm:w-auto">
        <a href="end-category-add.php" class="block text-center">Add New</a>
      </button>
    </div>
    ...
    <!-- Controls and search -->
    <form method="GET" action="end-category.php" class="flex flex-col sm:flex-row justify-between items-center mb-4">
      ...
      <div class="flex items-center w-full sm:w-auto">
        ...
        <input type="text" name="search" id="searchInput" class="border rounded px-2 py-1 w-full sm:w-64" 
               value="<?php echo htmlspecialchars($searchTerm); ?>" placeholder="Search by End Category Name..." />
    ...
    <div class="overflow-x-auto bg-white rounded shadow">
      <table class="w-full bg-white border-collapse">
        <thead>
          <tr class="bg-gray-50">
            <th class="py-2 px-4 border text-left w-16 relative">
              <a href="<?php echo getSortUrl('ecat_id', $sortColumn, $sortOrder); ?>" class="flex items-center justify-between">
                # 
                <span class="ml-1">
                  <?php echo getSortIcon('ecat_id', $sortColumn, $sortOrder); ?>
                </span>
              </a>
            </th>
            <th class="py-2 px-4 border text-left relative">
              <a href="<?php echo getSortUrl('ecat_name', $sortColumn, $sortOrder); ?>" class="flex items-center justify-between">
                End Level Categories 
                <span class="ml-1">
                  <?php echo getSortIcon('ecat_name', $sortColumn, $sortOrder); ?>
                </span>
              </a>
            </th>
            <th class="py-2 px-4 border text-left relative">
        ...
        <tbody id="end-categoryTableBody">
          <?php 
          if (mysqli_num_rows($result) > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                  echo '<tr class="hover:bg-gray-50">';
                  echo '<td class="py-2 px-4 border">' . $row['ecat_id'] . '</td>';
                  echo '<td class="py-2 px-4 border">' . htmlspecialchars($row['ecat_name']) . '</td>';
              ...
              echo '<tr><td colspan="3" class="py-4 px-4 text-center">No End Level Categorys found</td></tr>';
          }
          ?>
        </tbody>
      </table>
